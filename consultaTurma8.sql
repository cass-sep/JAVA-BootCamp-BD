...
SELECT ioc.idOrdemCompra, ioc.idMaterial, ioc.quantidade, ioc.valor, m.nome as 'Material', f.nome as 'Fornecedor'
...
SELECT m.nome as 'Material', f.nome as 'Fornecedor', ioc.valor, ioc.quantidade, ioc.quantidade * ioc.valor as 'SubTotal'
...
SELECT oc.id as 'Compra', oc.data as 'Data da Compra', f.nome as 'Fornecedor', oc.idFornecedor, SUM(ioc.quantidade * ioc.valor) as 'SubTotal' FROM ordem_compra as oc, fornecedor as f, item_ordem_compra as ioc WHERE f.id = oc.idFornecedor AND ioc.idOrdemCompra = oc.id GROUP BY oc.id;
